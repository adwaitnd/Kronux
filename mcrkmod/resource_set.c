...
///* FIXME 6/14/2013 hyos: Is this condition necessary? 
...
int rk_resource_set_detach_process_in_kernel_mode(int rd, pid_t pid, bool called_by_exit)
{
	rk_resource_set_t rset;
	struct task_struct *task;
	struct task_struct *tmptask;
...
#ifdef CONFIG_RK_MEM
	if (rset->mem_reserve) {
		mem_reserve_detach_process_in_kernel_mode(rset->mem_reserve->reserve, task, called_by_exit);
	}
#endif
	...
	if (!called_by_exit) rk_exit(task);
...
{
	return rk_resource_set_detach_process_in_kernel_mode(rd, pid, false);
}
...
asmlinkage int sys_rk_resource_set_destroy(int rd)
{
	...
	while (1) {
		...
		if (rset->task_list.next != &rset->task_list) {
			...
			rk_resource_set_detach_process_in_kernel_mode(rd, pid, false);
		}
 ...
 * 
 * Called by
 * - rk_resource_set_detach_process_in_kernel_mode
