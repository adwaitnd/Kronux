...
// Called by 
// - rk_exit
void rk_mutex_task_exit(struct task_struct *task)
{
	unsigned long flags;
	rk_mutex_t mutex;
	int mid;
...
// - This will be freed when the task exits (rk_mutex_task_exit)
//
// Called by
// - rk_mutex_open
...
int __rk_intervm_mutex_lock_handler(int type, int mid, int task_id, int task_prio, int is_trylock)
{
	unsigned long flags;
	rk_intervm_mutex_t mutex;
	rk_reserve_t rsv;
	...
	for (i = 0; i < mutex->wait_list_size; i++) {
		...
		if (mutex->wait_list[i].vcpu->rt_priority == current->rt_priority
		    && mutex->wait_list[i].task_prio >= task_prio) continue;
		// Insert here
...
int rk_intervm_mutex_lock_handler(int type, int mid, int task_id, int task_prio)
{
	return __rk_intervm_mutex_lock_handler(type, mid, task_id, task_prio, FALSE);
}

int rk_intervm_mutex_trylock_handler(int type, int mid, int task_id, int task_prio)
{
	return __rk_intervm_mutex_lock_handler(type, mid, task_id, task_prio, TRUE);
}
