...
/* Timeline Clockevent code */ //Added by Sandeep D'souza on 8 October 2015

static struct omap_dm_timer clkev_timeline;
static struct clock_event_device clockevent_timeline;
...
static struct irqaction omap2_gp_timer_irq_timeline = {
	.name		= "timeline_timer",
	.flags		= IRQF_TIMER | IRQF_IRQPOLL,
	.handler	= omap2_gp_timer_interrupt_timeline,
};
...
static void omap2_gp_timer_set_mode_timeline(enum clock_event_mode mode,
				    struct clock_event_device *evt)
{
	u32 period;
	...
	switch (mode) {
	case CLOCK_EVT_MODE_PERIODIC:
		period = clkev_timeline.rate / HZ;
		period -= 1;
		/* Looks like we need to first set the load value separately */
		__omap_dm_timer_write(&clkev_timeline, OMAP_TIMER_LOAD_REG,
				      0xffffffff - period, OMAP_TIMER_POSTED);
		__omap_dm_timer_load_start(&clkev_timeline,
					OMAP_TIMER_CTRL_AR | OMAP_TIMER_CTRL_ST,
					0xffffffff - period, OMAP_TIMER_POSTED);
		break;
	case CLOCK_EVT_MODE_ONESHOT:
						...
						const char *property)
{
	...
	__omap_dm_timer_int_enable(&clkev_timeline, OMAP_TIMER_INT_OVERFLOW);

	clockevent_timeline.cpumask = cpu_possible_mask;
	clockevent_timeline.irq = omap_dm_timer_get_irq(&clkev_timeline);
	clockevents_config_and_register_timeline(&clockevent_timeline, clkev_timeline.rate,
					3, /* Timer internal resynch latency */
					0xffffffff); // A new function added by Sandeep D'souza to clockevents.c
...
{}
#endif
// Modified by Sandeep D'souza omap_gp_clockevent_init_timeline added
